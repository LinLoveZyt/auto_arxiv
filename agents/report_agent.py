...
2.  **【最关键任务】图片选择:**
    ...
    -   **性能图 (performance_image):** 找出标题最能展示实验结果、性能对比或消融研究的图表。将其**精确的 `path`** 复制并填入输出的 `performance_image` 字段。
...
{{
  ...
  "selection_justification": "为何收录此论文的理由，直接复制输入信息中的“筛选理由”",
  "classification": {{
      "domain": "论文的领域",
      "task": "论文的任务"
  }},
...
def generate_report_json_for_paper(
    ...
    """
    为单篇论文生成结构化的JSON报告，包括分析、图片选择和筛选理由。
    """
    ...
    image_summary_str = json.dumps(image_summary_list, indent=2, ensure_ascii=False) if image_summary_list else "No images with captions found."

    current_config = config_module.get_current_config()
    target_model = current_config['OLLAMA_MODEL_NAME']
    
    # 默认为英文模板，这里我们假设主要使用中文模板，如果需要英文，也需对应修改
    ...
    required_keys = ["title", "authors", "arxiv_id", "selection_justification", "analysis", "images"]
